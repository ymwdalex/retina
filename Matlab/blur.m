function E = blur(A,sigma,shiftrow,shiftcol,minRC,maxRC)
...
for i = 1:nconn
    ...
    xrange = truncateNegIndex([start(i,2)-radius:stop(i,2)+radius]-shiftcol,cA);
    yrange = truncateNegIndex([start(i,1)-radius:stop(i,1)+radius]-shiftrow,rA);        
    ...
    xrange1 = start(i,2)-radius:stop(i,2)+radius;   
    yrange1 = start(i,1)-radius:stop(i,1)+radius;
    ...
    xrange2 = truncateNegIndex([start(i,2)-radius:stop(i,2)+radius],cA);
    yrange2 = truncateNegIndex([start(i,1)-radius:stop(i,1)+radius],rA);        
    Z1 = zeros(length(yrange1),length(xrange1));
    rowFrom = abs(yrange1(1) - yrange2(1)) + 1;
    colFrom = abs(xrange1(1) - xrange2(1)) + 1;
    Z1(rowFrom:rowFrom+length(yrange2)-1,colFrom:colFrom+length(xrange2)-1) = B(yrange2,xrange2);
